	...
	for grp = 1, 4 do
		...
		for index, player in pairs(raid[grp]) do
			...
			if (UnitIsConnected(player)) then -- add icons for 1st and 2nd soak and debuff & CHECK DEBUFF DOESNT TIME OUT NEXT 2s
				...
				elseif (not IRT_UnitDebuff(player, spellIDs["Sap"]) and count < 1)then
					C_ChatInfo.SendAddonMessage("IRT_HD", grp, "WHISPER", player);
					count = count + 1;
					printText = printText .. "|cFF00FF00" .. playerShort .. "|cFFFFFFFF";
				elseif (count == 1 and not IRT_UnitDebuff(player, spellIDs["Sap"])) then
					printText = printText .. "|cFFFFF200" .. playerShort .. "|cFFFFFFFF";
...
f:SetScript("OnEvent", function(self, event, ...)
	...
	elseif (event == "CHAT_MSG_ADDON" and IRT_HungeringDestroyerEnabled and inEncounter) then
		local prefix, msg, channel, sender = ...;
		if (prefix == "IRT_HD") then
			if (msg:match("soon")) then
				local soakTime = time() + 12;
				local text, mark = strsplit(" ", msg);
				IRT_PopupShow("\124TInterface\\TargetingFrame\\UI-RaidTargetingIcon_"..mark..":30\124t".." MOVE TO " .. groupIcons[mark] .. ", SOAK IN: |cFFFFFFFF" .. "12" .. "\124TInterface\\TargetingFrame\\UI-RaidTargetingIcon_"..mark..":30\124t", 1);
				C_Timer.NewTicker(1, function()
					local timeLeft = math.floor(soakTime - time());
					IRT_PopupShow("\124TInterface\\TargetingFrame\\UI-RaidTargetingIcon_"..mark..":30\124t".." MOVE TO " .. groupIcons[mark] .. ", SOAK IN: |cFFFFFFFF" .. timeLeft .. "\124TInterface\\TargetingFrame\\UI-RaidTargetingIcon_"..mark..":30\124t", 1);
					if (timeLeft <= 3) then
